...
const DEFAULT_OPTIONS = {
    ...
    setTimeout: (func, delay) => setTimeout(func, delay),
...
class JSONBird extends Duplex {
     ...
     * @param {boolean} [includeErrorStack=false] If `true` and `error.data` is `undefined`, the resulting `data` object will contain
     *        a "javascriptError" objecting containing the `fileName`, `lineNumber`, `columnNumber` and `stack` of the `error`.
    ...
    static errorToResponseObject(error, includeErrorStack) {
        ...
        if (includeErrorStack) {
            ...
            if (typeof responseError.data === 'object' &&
                ...
                responseError.data.javascriptError === undefined
            ) {
                responseError.data = Object.assign({
                    javascriptError: {
                        name: error.name,
    ...
    constructor(optionsArg = {}) {
        ...
        if (options.writableMode !== 'object' &&
            ...
            throw Error('JSONBird: Invalid value for `writableMode` option');
        }

        if (options.readableMode !== 'object' &&
            options.readableMode !== 'json-stream' &&
            options.readableMode !== 'json-message') {
            throw Error('JSONBird: Invalid value for `readableMode` option');
        }

        if (typeof options.endOfJSONWhitespace !== 'string') {
            throw Error('JSONBird: The `endOfJSONWhitespace` option must be a string');
        }
        ...
        this.on('finish', () => {
            ...
            for (let pendingData of clientPendingMap.values()) {
                const {reject, timer} = pendingData;
    ...
    method(name, func) {
        if (typeof name !== 'string') {
            throw Error('JSONBird: First argument (name) must be a string');
        }

        if (typeof func !== 'function') {
            throw Error('JSONBird: Second argument (func) must be a function');
        }
    ...
    methods(objectOrMap) {
        if (!objectOrMap || typeof objectOrMap !== 'object') {
            throw Error('JSONBird: First argument (objectOrMap) must be an object, or a Map');
        }
    ...
    callLocal(name, ...args) {
        return prom(Promise.resolve().then(() => {
            ...
            if (name.startsWith('rpc.')) { // reserved
                throw new RPCRequestError(Error('JSONBird: Method not found'), -32601);
            }
    ...
    callLocalAndSendResponse(id, name, ...args) {
        if (!JSONBird.isValidID(id)) {
            return Promise.reject(Error('JSONBird: First argument is not a valid id'));
        }
    ...
    call(nameOrOptions, ...args) {
        ...
        if ((typeof nameOrOptions !== 'string' && typeof nameOrOptions !== 'object') || nameOrOptions === null) {
            return Promise.reject(new Error('JSONBird: First argument must be a string or an object with at least a "name" property'));
        }
        ...
        if (clientPendingMap.has(id)) {
            return Promise.reject(new RPCRequestError(Error('JSONBird: Generated request "id" is not unique!'), -32603));
        }

        let pendingData;

        const responsePromise = new Promise((resolve, reject) => {
            pendingData = {resolve, reject, timer: 0};

            pendingData.timer = timeout && this.setTimeout(
                () => {
                    pendingData.timer = 0;
        ...
        Promise.resolve().then(() => {
            if (this.writableMode === 'object') {
                ...
                if (typeof chunk !== 'object' || Buffer.isBuffer(chunk)) {
                    throw Error(`JSONBird: Assertion Error: expected an object during _write()`);
                }
            ...
            if (this.writableMode === 'json-message') {
                ...
                if (typeof chunk !== 'string' && !Buffer.isBuffer(chunk)) {
                    throw Error(`JSONBird: Assertion Error: expected a string or Buffer during _write()`);
                }
            ...
            /* istanbul ignore next */
            throw Error(`JSONBird: Assertion Error: Invalid this.readableMode during _write(): ${this.readableMode}`);
        })
    ...
    handleRequestObject(object) {
        ...
        return Promise.resolve().then(() => {
            ...
            if (!JSONBird.isValidVersion(object.jsonrpc)) {
                throw new RPCRequestError(Error('JSONBird: Invalid Request: given "jsonrpc" version is not supported'), -32600);
            }

            if (!JSONBird.isValidMethodName(object.method)) {
                throw new RPCRequestError(Error('JSONBird: Method not found: "method" attribute must be a string'), -32601);
            }
            ...
            if ('params' in object) {
                if (JSONBird.isValidParams(object.params)) {
                    ...
                    throw new RPCRequestError(Error('JSONBird: Invalid Request: "params" must be an array or object'), -32600);
                }
            }

            if ('id' in object) {
                if (id === null) {
                    throw new RPCRequestError(Error('JSONBird: Invalid Request: "id" must be a number or a string'), -32600);
                }
    ...
    handleResponseObject(object) {
        return Promise.resolve().then(() => {
            if (!JSONBird.isValidVersion(object.jsonrpc)) {
                ...
                throw new RPCResponseError(Error('JSONBird: Invalid Response: "jsonrpc" property must be "2.0"'));
            }

            if (!JSONBird.isValidID(object.id)) {
                throw new RPCResponseError(Error('JSONBird: Invalid Response: "id" property must be a number or string'));
            }

            if (!('result' in object || typeof object.error === 'object')) {
                throw new RPCResponseError(Error('JSONBird: Invalid Response: Must have a "error" or an "result" property'));
            }

            if ('result' in object && 'error' in object) {
                throw new RPCResponseError(Error('JSONBird: Invalid Response: The "error" and "result" properties are both present'));
            }
            ...
            if (!pendingEntry) {
                throw new RPCResponseError(Error('JSONBird: Invalid Response: Unknown id'));
            }
            ...
            if ('result' in object) {
                ...
                if (this.receiveErrorStack &&
                    typeof data === 'object' &&
                    data &&
                    typeof data.javascriptError === 'object' &&
                    data.javascriptError
                ) {
                    error.parseRemoteJavascriptError();
                }
    ...
    sendObject(object) {
        ...
        return Promise.resolve().then(() => {
            ...
            if (this.readableMode === 'object') {
                ...
                throw Error(`JSONBird: Assertion Error: Invalid this.readableMode during sendObject(): ${this.readableMode}`);
            }
        });
    }
    ...
    startPinging() {
        if (this.ended || this.finished) {
            throw Error('JSONBird: startPinging(): This stream has been ended or finished');
        }
